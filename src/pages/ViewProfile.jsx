...
const ViewProfile = () => {
    ...
    const [formData, setFormData] = useState({ username: "", email: "", mobile: "", address: "", name: "" });
    const [initialFormData, setInitialFormData] = useState({ username: "", email: "", mobile: "", address: "", name: "" });
    const [errors, setErrors] = useState({ email: "", mobile: "", address: "", name: "" });
        ...
        const fetchProfile = async () => {
            ...
            try {
                ...
                if (response.data.success) {
                    ...
                    setSnack({ open: true, severity: "success", message: "Profile fetched successfully" });
                } else {
                    throw new Error("Failed to fetch profile");
                }
            } catch (error) {
                setSnack({ open: true, severity: "error", message: error.response?.data?.message || "Failed to fetch profile" });
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${apiConfig.BASE_URL}/update/profile`, payload, {
                headers: { Authorization: `Bearer ${token}` },
            });
            if (response.data.success) {
            ...
            setSnack({ open: true, severity: "error", message: error.response?.data?.message || error.message });
        } finally {
            setUpdating(false);
        }
    };
        ...
        <Box sx={{ maxWidth: 600, mx: "auto" }}>
            ...
            <List disablePadding>
                <ListItem disableGutters sx={{ py: 0.8 }}>
                    ...
                    <ListItemText primary={profile.email || "Not provided"} secondaryTypographyProps={{ fontSize: 14 }} />
                </ListItem>
                <ListItem disableGutters sx={{ py: 0.8 }}>
                    <ListItemIcon sx={{ minWidth: 50, color: "text.secondary" }}>
                        <PhoneIcon />
                    </ListItemIcon>
                    <ListItemText primary={profile.mobile || "Not provided"} secondaryTypographyProps={{ fontSize: 14 }} />
                </ListItem>
            </List>
            <Stack direction="row" spacing={2} mt={4}>
                ...
                <Button variant="contained" color="primary" onClick={() => navigate(ROUTES.RESET_PASSWORD)}>
                    Reset Password
                </Button>
            </Stack>
            <ModalComponent
                ...
                content={
                    <Box display="flex" flexDirection="column" gap={2} p={2}>
                        ...
                        <Box display="flex" justifyContent="flex-end" gap={2} mt={2}>
                            <Button onClick={() => setModalOpen(false)} variant="outlined" disabled={updating}>
                                Cancel
                            </Button>
                            <Button onClick={handleUpdate} variant="contained" disabled={updating || !isFormChanged()}>
